...
const COLUMN_TO_LANE_2: Record<number, number> = { 0: 0, 1: 0, 2: 1, 3: 1 };
...
export function condenseTo2Lanes(beatMap: BeatMap): BeatMap {
	return {
		...beatMap,
		notes: beatMap.notes.map((note) => ({
			...note,
			column: COLUMN_TO_LANE_2[note.column] ?? 0
		})),
		obstacles: beatMap.obstacles.map((obs) => ({
			...obs,
			column: COLUMN_TO_LANE_2[obs.column] ?? 0,
			width: Math.min(obs.width, 1)
		}))
	};
}
